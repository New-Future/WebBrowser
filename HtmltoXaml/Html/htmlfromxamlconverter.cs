...
namespace Html
{
    ...
    internal static class HtmlFromXamlConverter
    {
            ...
            while (xamlReader.MoveToNextAttribute())
            {
                ...
                switch (xamlReader.Name)
                {
                    ...
                    case "LineHeight":
                        break;
                    case "TextIndent":
                        css = "text-indent:" + ParseSize(xamlReader.Value) + ";";
                        break;
                    case "TextAlignment":
                        css = "text-align:" + ParseSize(xamlReader.Value) + ";";
                        break;
                    case "IsKeptTogether":
                        break;
                    case "IsKeptWithNext":
                    ...
                    // ----------------
                    case "Width":
                        css = "width:" + ParseSize(xamlReader.Value) + ";";
                        break;
                    case "ColumnSpan":
        ...
        public static string ParseSize(string styleValue)
        {
            ...
            if (startIndex < styleValue.Length && Char.IsDigit(styleValue[startIndex]))
            {
                while (startIndex < styleValue.Length && (Char.IsDigit(styleValue[startIndex]) || styleValue[startIndex] == '.'))
                {
                    startIndex++;
                }

                string number = styleValue.Substring(0, startIndex).Trim();

                string unit = styleValue.Substring(startIndex).Trim();
